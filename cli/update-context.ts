...
interface SimpleStore {
  listProjects(): Promise<Array<{ id: string; name: string; path: string; last_updated: string }>>;
...
type StoreConfig = ApiConfig | SupabaseConfig;
 ...
 */
function parseOpenContextServer(config: any): { url: string; apiKey: string } | null {
  // Try both "open-context" and "context-explorer" as server names
  const server =
    config?.mcpServers?.["open-context"] ||
 ...
 */
function readMcpJson(filePath: string): any | null {
  try {
 ...
 */
function parseClaudeJson(projectPath: string): { url: string; apiKey: string } | null {
 ...
 */
function getApiConfig(projectPath: string): ApiConfig | null {
  let serverUrl: string | null = null;
  let apiKey: string | null = null;

  // 1. Check project .mcp.json (committed to git)
  const projectMcp = readMcpJson(path.join(projectPath, ".mcp.json"));
  if (projectMcp) {
  ...
  if (!serverUrl || !apiKey) {
    ...
    if (claudeConfig) {
      if (!serverUrl) serverUrl = claudeConfig.url;
      if (!apiKey) apiKey = claudeConfig.apiKey;
    }
  }
...
function resolveConfig(projectPath: string): StoreConfig {
  ...
  if (hasServer && !hasServer.apiKey) {
    throw new Error(
      "found open-context server in .mcp.json but no API key. " +
...
function createRemoteStore(config: ApiConfig): SimpleStore {
  ...
  return {
    ...
    async getProject(id: string) {
      try {
        ...
        return {
          ...project,
          modules: (project.modules || []).map((m: Record<string, unknown>) => ({
            id: m.id as string,
...
function createSupabaseStore(config: SupabaseConfig): SimpleStore {
...
function createStore(config: StoreConfig): SimpleStore {
  if (config.type === "remote") return createRemoteStore(config);
...
async function findProject(store: SimpleStore, projectPath: string) {
