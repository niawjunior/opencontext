...
function getLogDir() {
    const dir = node_path_1.default.join(node_os_1.default.homedir(), ".open-context", "logs");
...
function findClaude() {
    try {
        const result = (0, node_child_process_1.execFileSync)("which", ["claude"], {
            encoding: "utf-8",
            timeout: 5000,
        }).trim();
        return result || null;
...
function hasMcpJson(projectPath) {
    try {
        ...
        for (const server of Object.values(config?.mcpServers || {})) {
            ...
            if (args?.some((a) => a.includes("open-context") || a.includes("context-explorer"))) {
                return true;
            }
        }
        return false;
...
function getMcpServerName(projectPath) {
    try {
        const content = node_fs_1.default.readFileSync(node_path_1.default.join(projectPath, ".mcp.json"), "utf-8");
        const config = JSON.parse(content);
...
function getGitDiff(projectPath, modulePath) {
    try {
        ...
        const diff = (0, node_child_process_1.execFileSync)("git", diffArgs, {
            cwd: projectPath,
            encoding: "utf-8",
            timeout: 10_000,
            maxBuffer: 1024 * 1024,
...
function buildPrompt(mod, diff, projectId, projectName, mcpServerName) {
...
async function main() {
    ...
    const logFile = node_path_1.default.join(getLogDir(), `context-update-${Date.now()}.log`);
    ...
    for (const mod of payload.modules) {
        ...
        try {
            ...
            const result = (0, node_child_process_1.execFileSync)(claudePath, [
                "-p", prompt,
                "--output-format", "text",
...
//# sourceMappingURL=smart-context-update.js.map